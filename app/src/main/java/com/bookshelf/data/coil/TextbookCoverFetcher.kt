 ...
 *
 * It uses [Textbook.thumbnailUrl] if custom cover is not set by the user.
 ...
 */
class TextbookCoverFetcher(
    private val url: String?,
    private val isLibraryTextbook: Boolean,
    ...
    private suspend fun httpLoader(): FetchResult {
        ...
        val libraryCoverCacheFile = if (isLibraryTextbook) {
    ...
    ) : Fetcher.Factory<Textbook> {

        override fun create(data: Textbook, options: Options, imageLoader: ImageLoader): Fetcher {
            return TextbookCoverFetcher(
                url = data.thumbnailUrl,
                isLibraryTextbook = data.favorite,
                options = options,
    ...
    class TextbookCoverFactory(
        private val callFactoryLazy: Lazy<Call.Factory>,
        private val coverCache: CoverCache,
        private val sourceManager: SourceManager,
    ) : Fetcher.Factory<TextbookCover> {

        override fun create(data: TextbookCover, options: Options, imageLoader: ImageLoader): Fetcher {
            return TextbookCoverFetcher(
                url = data.url,
                isLibraryTextbook = data.isFavorite,
                options = options,
                coverFileLazy = lazy { coverCache.getCoverFile(data.url) },
                customCoverFileLazy = lazy { coverCache.getCustomCoverFile(data.textbookId) },
                diskCacheKeyLazy = lazy { imageLoader.components.key(data, options)!! },
