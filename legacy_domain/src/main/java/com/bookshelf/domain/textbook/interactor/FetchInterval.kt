...
class FetchInterval(
    ...
    suspend fun toTextbookUpdate(
        ...
        val interval = manga.fetchInterval.takeIf { it < 0 } ?: calculateInterval(
            chapters = getChaptersByTextbookId.await(manga.id, applyScanlatorFilter = true),
            zone = timeZone,
        )
        ...
        return TextbookUpdate(id = manga.id, nextUpdate = nextUpdate, fetchInterval = interval)
    }
